...
function K = F_kernel(anisotropy,kernel_choice,beta,phi,m,...
 ...
 %_________________________________________________________
     if kernel_choice == 5 %EJZ FD
        ...
        K=zeros(N,N); epsii=0.001;
        cn= (1+epsii)*(8*S0*S2); % we choose mobility equal to anisotropy

         for i= 1:N
            for j=1:N
                 thr = atan2(YF(i,j),XF(i,j));
                 n_xi=sqrt((XF(i,j))^2 + (YF(i,j))^2);
                 
                 gk= (1 + beta*cos(m*(thr+phi)));
                 mu=gk;%mobility

                 alpha= (gk*sqrt(cn) + (sqrt(cn*gk*gk - 8*S0*S2*gk*mu)))*(pi*n_xi)/(S2*sqrt(cn));
                 Beta = (gk*sqrt(cn) - (sqrt(cn*gk*gk - 8*S0*S2*gk*mu)))*(pi*n_xi)/(S2*sqrt(cn));
%_________________________________________________________
