   ...
   xw_n=xw.*sqrt(Ratio_area); yw_n=yw.*sqrt(Ratio_area);%analytical solution for given set up
    ...
    %Main loop of thresholding
       while movement > 0 && final_time > TT
           ...
           %Evolution plot for gpi=12 & dti=5
          if Evolution_plot==1 &&  gpi==gpiEV && dti==5
              if iteration==10 || iteration==25 || iteration==50 ||...
                      iteration==192 
                  figure(1)
            ...
            if Evolution_plot==1 &&  gpi==gpiEV && dti==5
                figure(1)
                 contour(X,Y,P_initial,[0.5,0.5],'k','LineWidth',3);  hold on  
                  ...
                  plot(xw_n,yw_n,'Color','black','LineWidth',4);
                  line([-3.5,3.5],[0,0],'Color','black','LineWidth',4);
                  axis equal, 
                  axis([-2 2 0 3]) 
                  %maximise the plot and save
                set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
                set(gcf, 'Toolbar', 'none', 'Menu', 'none');
                saveas(gcf,'Fig6_6.png')
            end
...
title('\fontsize{40} Bonnetier-Bretin-Chambolle');
xlim([10^-3 10^0])

%maximise the plot and save
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
set(gcf, 'Toolbar', 'none', 'Menu', 'none');
saveas(gcf,'Fig6_1.png')
hold off
